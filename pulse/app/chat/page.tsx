...
export default function Home() {
  ...
  const userId = user?.emailAddresses[0]?.emailAddress || "guest-" + Math.random().toString(36).substr(2, 9);
  ...
  return (
    ...
    <motion.button
      ...
      className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center hover:shadow-lg transition-all overflow-hidden"
    >
      {user?.imageUrl ? (
        <Image
          src={user.imageUrl}
          ...
          {user ? (
            <>
              <div className="px-4 py-3 border-b border-blue-100/30">
                <p className="text-sm font-serif text-foreground font-medium truncate">
                  {user.fullName}
                </p>
                <p className="text-xs text-foreground/60 font-serif truncate">
